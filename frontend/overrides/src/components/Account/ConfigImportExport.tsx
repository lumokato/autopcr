...
import { loadFavoriteMap, normalizeFavoritesForAreas, saveFavoriteMap } from '../../utils/favorites';
...
const ConfigImportExport = ({ alias, areas, onImportSuccess }: ConfigIOProps) => {
    ...
    const onExport = () => {
        ...
        void Promise.all(
            ...
            const favMap = normalizeFavoritesForAreas(
                loadFavoriteMap(alias),
                configs.map((config, index) => ({ key: areas[index].key, order: config.order })),
            );
    ...
    const realImport = async (rawCfg: string) => {
        try {
            ...
            saveFavoriteMap(
                alias,
                normalizeFavoritesForAreas(
                    importedFav,
                    configItems.map((config, index) => ({ key: areas[index].key, order: config.order })),
                ),
            );
