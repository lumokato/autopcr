...
class user_info(Module):
    ...
    async def do_task(self, client: pcrclient):
        ...
        total_power = profile.user_info.total_power
        total_power_base = sum(data.get_unit_power(unit) for unit in data.unit)
        total_power_ex = total_power - total_power_base
        ...
        self._log(f"全角色战力：{format_number(total_power)}(ex: {format_number(total_power_ex)})")
